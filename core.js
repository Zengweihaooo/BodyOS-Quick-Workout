...
export function sessionSummary(session) {
  ...
  const volumes = session.sets.map((set) => calculateSetVolume(set)).filter((value) => value != null);
  return {
...
export function buildBodyCandidate(session) {
  ...
  const exercises = [...grouped.entries()].map(([exerciseId, sets], orderIndex) => ({
    ...
    confidence: { exercise: 1 }, sets: sets.map((set, index) => {
      ...
      if (item.loadMode === "assistance") result.assistanceWeight = item.weightValue;
      return result;
    }),
  }));
  return {
...
export function toMarkdown(session) {
  ...
  session.sets.forEach((set) => {
    ...
    const extras = [set.rir != null ? `RIR ${set.rir}` : "", set.rpe != null ? `RPE ${set.rpe}` : "", set.rer != null ? `RER ${set.rer}` : "", set.notes || ""].filter(Boolean).join(" · ");
