...
- 资料库以 Body OS canonical ID 为准，内置 39 个经人工核对的 dataset GIF、双语步骤，并为 8 个动作附上 wger 说明/视频或动作页。引用清单随 `core.js` 静态发布；方形 GIF 使用 `object-fit: contain` 完整展示，第三方媒体保持远程引用且不写入 Service Worker 缓存。
