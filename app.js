import { EXERCISE_CATALOG_VERSION, FALLBACK_EXERCISES, LOAD_LABELS, buildBodyCandidate, canonicalExerciseId, createExport, createSession, mergeExerciseCatalog, normalizeSet, restRemainingSeconds, sessionSummary, timerElapsedMs, toMarkdown, withoutExercise } from "./core.js";
...
function exerciseIcon(exercise) { const reduced = globalThis.matchMedia?.("(prefers-reduced-motion: reduce)").matches; const gif = safeExternalUrl(reduced ? (exercise.reference?.thumbnailUrl || exercise.reference?.gifUrl) : exercise.reference?.gifUrl, "dataset"); if (gif) return `<img class="exercise-gif" src="${escapeHTML(gif)}" alt="" loading="lazy" decoding="async" referrerpolicy="no-referrer">`; const map = { horizontal_push: "↗", vertical_push: "↑", horizontal_pull: "↙", vertical_pull: "↓", squat: "◇", hinge: "⌁", shoulder_abduction: "↔", shoulder_flexion: "⌃" }; return map[exercise.movementPattern] || "✦"; }
...
function renderPicker() {
  setScreenHeading(localeText("选择动作", "Choose exercise")); app.innerHTML = `<div class="picker-actions"><input class="search" id="search" type="search" placeholder="${localeText("搜索动作、器械或英文名", "Search exercise, equipment or Chinese name")}" autocomplete="off" aria-label="搜索动作"><button class="custom-action" id="addCustom">＋ ${localeText("自定义", "Custom")}</button></div><section class="category-guide"><div class="section-head"><h2>${localeText("按训练部位快速找", "Browse by body part")}</h2><span class="label">${localeText("内置 wger 常用动作", "Offline wger essentials")}</span></div><div class="chip-row"><button class="chip active" data-filter="">${localeText("全部", "All")}</button><button class="chip" data-filter="chest">${localeText("胸", "Chest")}</button><button class="chip" data-filter="back">${localeText("背", "Back")}</button><button class="chip" data-filter="legs">${localeText("腿", "Legs")}</button><button class="chip" data-filter="shoulders">${localeText("肩", "Shoulders")}</button><button class="chip" data-filter="arms">${localeText("手臂", "Arms")}</button></div></section><div class="card-list" id="exerciseList"></div>`;
  ...
  search.oninput = update; document.querySelectorAll("[data-filter]").forEach((chip) => chip.onclick = () => { document.querySelectorAll("[data-filter]").forEach((x) => x.classList.remove("active")); chip.classList.add("active"); filter = chip.dataset.filter; update(); }); $("#addCustom").onclick = () => navigate("custom"); update(); setTimeout(() => search.focus(), 50);
}
...
function renderEntry() {
  ...
  const reference = d.reference || {}, instructions = state.locale === "en" ? reference.instructionsEn : reference.instructionsZh, description = state.locale === "en" ? reference.wger?.descriptionEn : (reference.wger?.descriptionZh || reference.wger?.descriptionEn), wgerUrl = safeExternalUrl(reference.wger?.videoUrl || reference.wger?.pageUrl, "wger");
  const referencePanel = reference.datasetId ? `<details class="exercise-reference-panel"><summary>${localeText("动作资料与步骤", "Exercise guide")}</summary><div>${description ? `<p>${escapeHTML(description)}</p>` : ""}${Array.isArray(instructions) && instructions.length ? `<ol>${instructions.map((step) => `<li>${escapeHTML(step)}</li>`).join("")}</ol>` : ""}<small>dataset ${escapeHTML(reference.datasetId)}${reference.wger ? ` · wger ${reference.wger.matchType === "exact" ? localeText("已匹配", "matched") : localeText("通用参考", "reference")}` : ""}</small>${wgerUrl ? `<a href="${escapeHTML(wgerUrl)}" target="_blank" rel="noopener noreferrer" referrerpolicy="no-referrer">${localeText("在 wger 查看", "View on wger")} ↗</a>` : ""}</div></details>` : "";
