...
let state = { session: null, exercises: FALLBACK_EXERCISES, screen: "home", draft: null, selectedWatch: "", watchCandidates: [], importing: false, resetArmed: false, locale: "zh" };
...
function exerciseCard(exercise) { if (!exercise) return ""; const count = state.session.sets.filter((s) => s.exerciseId === exercise.id).length; const card = `<button class="exercise-card" data-exercise="${escapeHTML(exercise.id)}"><span class="exercise-icon">${exerciseIcon(exercise)}</span><span>${exerciseLabel(exercise)}<small>${escapeHTML(exercise.equipment || localeText("标准动作", "Standard"))} · ${LOAD_LABELS[exercise.loadMode] || localeText("重量", "Load")}${count ? ` · ${count} ${localeText("组", "sets")}` : ""}</small></span><span class="chevron">›</span></button>`; return count ? `<div class="exercise-manage-row">${card}<button class="exercise-delete" data-delete-exercise="${escapeHTML(exercise.id)}" aria-label="删除 ${escapeHTML(exercise.name)}">×</button></div>` : card; }
...
async function loadExerciseLibrary() {
  ...
  if (!canDirectBodyOs) { if (cached?.length) state.exercises = cached; return; }
  try { const response = await fetch("/api/training/snapshot"); if (!response.ok) return; const data = await response.json(); const items = data.exercise_library || data.exerciseLibrary || []; if (data.body_weight_kg != null) state.session.bodyWeightKg = Number(data.body_weight_kg); if (Array.isArray(items) && items.length) { const remote = items.map((item) => ({ id: item.id || item.exercise_canonical_id || item.canonical_id, name: item.canonical_name_zh || item.name || item.display_name, canonicalNameEn: item.canonical_name_en || "", equipment: item.equipment || "", movementPattern: item.movement_pattern || "", loadMode: item.default_load_mode || "total", executionMode: item.supports_unilateral_execution ? "unilateral" : item.supports_per_side_load ? "bilateral_simultaneous" : "bilateral", sideCount: item.supports_per_side_load ? 2 : 1 })).filter((x) => x.id && x.name); const seenIds = new Set(remote.map((item) => item.id)), seenNames = new Set(remote.map((item) => `${item.name}|${item.canonicalNameEn}`.toLowerCase())); state.exercises = [...remote, ...FALLBACK_EXERCISES.filter((item) => !seenIds.has(item.id) && !seenNames.has(`${item.name}|${item.canonicalNameEn}`.toLowerCase())), ...custom.filter((item) => !seenIds.has(item.id))]; await DB.set("exercise-library", state.exercises); } } catch { if (cached?.length) state.exercises = cached; }
}
